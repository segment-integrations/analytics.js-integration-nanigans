...
var Nanigans = module.exports = integration('Nanigans')
  ...
  .tag('track', '<img src="//api.nanigans.com/event.php?app_id={{ appId }}&type={{ type }}&name={{ name }}&user_id={{ userId }}&ut1={{ ut1 }}{{ custom }}">')
  .tag('product', '<img src="//api.nanigans.com/event.php?app_id={{ appId }}&type=purchase&name={{ name }}&user_id={{ userId }}&ut1={{ ut1 }}&sku={{ sku }}{{ custom }}">')
  .tag('add_to_cart', '<img src="//api.nanigans.com/event.php?app_id={{ appId }}&type=user&name={{ name }}&user_id={{ userId }}&ut1={{ ut1 }}&{{ products }}{{ custom }}">')
  .tag('purchase', '<img src="//api.nanigans.com/event.php?app_id={{ appId }}&type={{ type }}&name={{ name }}&user_id={{ userId }}&ut1={{ ut1 }}&unique={{ orderId }}&{{ products }}{{ custom }}">');
...
Nanigans.prototype.track = function(track) {
  ...
  for (var j = 0; j < events.length; j++) {
    ...
    each(function(pair) {
      var value;
      if (pair.key && pair.value) value = track.proxy('properties.' + pair.key);
      if (value) customQuery[pair.value] = value;
    }, custom);

    var customQueryString = qs.stringify(customQuery);
    if (customQueryString.length) params.custom = '&' + customQueryString;
