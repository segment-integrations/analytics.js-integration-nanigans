...
var Nanigans = module.exports = integration('Nanigans')
  ...
  .tag('add_to_cart', '<img src="//api.nanigans.com/event.php?app_id={{ appId }}&type=user&name={{ name }}&user_id={{ userId }}&ut1={{ ut1 }}&sku={{ sku }}">')
  .tag('add_to_cart_no_user_id', '<img src="//api.nanigans.com/event.php?app_id={{ appId }}&type=user&name={{ name }}&ut1={{ ut1 }}&sku={{ sku }}">')
  .tag('purchase', '<img src="//api.nanigans.com/event.php?app_id={{ appId }}&type=purchase&name={{ name }}&user_id={{ userId }}&ut1={{ ut1 }}&unique={{ orderId }}&{{ products }}">')
...
Nanigans.prototype.track = function(track) {
  ...
  for (var j = 0; j < events.length; j++) {
    ...
    switch (event.type) {
    case 'purchase':
      console.log('params in purchase', params);
    ...
    case 'user':
      ...
      case 'Product Added':
        ...
        params.products.sku = data.sku;
        params.products = qs.stringify(params.products);
        console.log('params in add_to_cart', params);
        params.userId ? this.load('add_to_cart', params) : console.log('helpful'); this.load('add_to_cart_no_user_id', params);
        break;
      default:
        console.log('user -> default track');
        params.userId ? this.load('track', params) : this.load('track_no_user_id', params);
        break;
      }
      break;
    default:
      console.log('params in default under user', params);
      params.userId ? this.load('track', params) : this.load('track_no_user_id', params);
      break;
    }
  }
